...
class Laporan extends BaseController
{
	...
	public function historiBayar(){
		...
		$htmlData .='<p>Berikut adalah histori pembayaran siswa <br/>NISN : '.$this->request->getPost('txtNoIndukSiswaNasional').' </p>';
	...
	public function penerimaanTampil(){
		...
		$htmlData .='<p>
		<a href="/laporan/excel/'.$this->request->getPost('txtTglBayar').'" class="btn btn-success btn-sm mr-2">Export Ke Excel</a>
		</p>';			
	...
	public function excel($tanggal){
		$this->bayar->join('siswa','siswa.nisn=pembayaran.nisn');
		$this->bayar->join('kelas','kelas.id_kelas=siswa.id_kelas');
		$data= array('title' => 'laporan ', 'listLaporan' => $this->bayar->where('pembayaran.tgl_bayar', $tanggal)->findAll());
