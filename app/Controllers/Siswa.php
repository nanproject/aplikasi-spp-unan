...
class Siswa extends BaseController
{
	...
	public function simpanSiswa(){
		...
		//cek siswa
		$cekSiswa = $this->siswa->where('nisn', $dataSiswa['nisn'])->findAll();
		if(count($cekSiswa) == 1){ //jika sudah ada
			return redirect()->to('/siswa')->with('pesan','<div class="alert alert-danger">Gagal! Siswa sudah ada</div>');
		}else{ //jika belum ada
			$this->siswa->insert($dataSiswa);
		}

		
		return redirect()->to('/siswa')->with('pesan','<div class="alert alert-success">Siswa baru berhasil di tambahkan.</div>');
	}
	...
	public function hapusSiswa($nisn){
		...
		if(siswaInBayar($nisn)==0){
			...
			return redirect()->to('/siswa')->with('pesan','<div class="alert alert-success">Siswa berhasil di hapus.</div>');
		} else {
			return redirect()->to('/siswa')->with('pesan','<div class="alert alert-danger">Gagal Hapus ! Siswa sudah melakukan pembayaran </div>');
		}
	}
	...
	public function updateSiswa(){
		...
		return redirect()->to('/siswa')->with('pesan','<div class="alert alert-success">Data siswa berhasil di update.</div>');;
	}
	...
	public function loginSiswa(){
		...
		if(count($dataSiswa)==1){
			...
			return redirect()->to('/')->with('gagal-login','<div class="alert alert-danger">username atau password salah</div>');
		}
	}
